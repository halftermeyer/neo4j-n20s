...
| `n20s.graph.projectTemplate(name, template, row, [ifExists])` | Aggregating function: project nodes, relationships, paths, entity lists, or maps into a named graph via a declarative JSON template (R2RML-style IRI templates). Default ifExists: `'replace'` |
